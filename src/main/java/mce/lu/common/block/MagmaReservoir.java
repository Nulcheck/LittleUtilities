...
public class MagmaReservoir extends BlockBase {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand,
		...
		if (!player.getHeldItemMainhand().isEmpty() && player.getHeldItemMainhand().getItem() == Items.BUCKET) {
			...
			player.inventory.addItemStackToInventory(new ItemStack(Items.LAVA_BUCKET, 1));
			return true;
		}

		// Empties water bucket
		if (!player.getHeldItemMainhand().isEmpty() && player.getHeldItemMainhand().getItem() == Items.LAVA_BUCKET) {
			player.getHeldItemMainhand().shrink(1);
