...
public class LiquidPigment extends BlockFluidClassic {
	float red, green, blue = 1f;

	public LiquidPigment(Fluid fluid, Material mat) {
		super(fluid, mat);
	}

	public LiquidPigment setColor(EnumDyeColor c) {
		return setColor(((c.getColorValue() >> 16) & 255) / 255f, ((c.getColorValue() >> 8) & 255) / 255f,
				(c.getColorValue() & 255) / 255f);
	}

	public LiquidPigment setColor(int c) {
		return setColor(((c >> 16) & 255) / 255f, ((c >> 8) & 255) / 255f, (c & 255) / 255f);
	}

	public LiquidPigment setColor(float redIn, float greenIn, float blueIn) {
		this.red = redIn;
		this.green = greenIn;
		this.blue = blueIn;
